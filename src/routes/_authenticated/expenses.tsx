...
function ExpensesPage() {
  ...
  const [editForm, setEditForm] = useState({
    ...
    project_id: "",
    asset_id: "",
    asset_expense_type: "maintenance",
  ...
  const filtered = useMemo(() => rows.filter((e: any) => {
    ...
    return (
      ...
      (e.projects?.name ?? "").includes(search) ||
      (e.assets?.asset_name ?? "").includes(search) ||
      (e.creditor_name ?? "").includes(search) ||
      (e.expense_scope === "general" && "مصروف عام".includes(search))
    );
  }), [rows, search]);
  ...
  function openNew() {
    setForm({
      ...
      asset_id: "",
      asset_expense_type: "maintenance",
      asset_cost_treatment: "operating_expense",
      category_id: "",
      amount: "",
      expense_date: new Date().toISOString().slice(0, 10),
      description: "",
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!online) {
      ...
      if (file || excelFile) {
        return toast.error("لا يمكن رفع المرفقات أوفلاين", {
          description: "احفظ بدون مرفقات أو انتظر عودة الاتصال",
        });
      }
      ...
      try {
        const linkedName =
          form.expense_scope === "project"
            ? (projects ?? []).find((p: any) => p.id === form.project_id)?.name ?? "مشروع"
            : form.expense_scope === "asset"
              ? (assets ?? []).find((a: any) => a.id === form.asset_id)?.asset_name ?? "أصل"
              : "مصروف عام";

        await enqueue({
          type: "expense.create",
          label: `${linkedName} — ${form.amount} د.ل`,
          payload: {
        ...
        toast.success("تم حفظ المصروف في الطابور", {
          description: "سيُرسل تلقائياً عند عودة الاتصال",
        });
    ...
    try {
      ...
      const { error } = await supabase.rpc("create_expense_v3", {
        ...
        _allocations: isPayable ? [] : allocations.map((a) => ({
          funding_check_id: a.funding_check_id,
          amount: Number(a.amount),
        })),
  ...
  async function onSaveEditExp(ev: React.FormEvent) {
    ...
    try {
      const { error } = await supabase.rpc("update_expense_atomic", {
        ...
        _allocations: editAllocations.map((a) => ({
          funding_check_id: a.funding_check_id,
          amount: Number(a.amount),
        })),
      } as any);
      ...
      toast.success("تم تحديث المصروف", {
        description: "تم تسجيل التغيير في سجل المراجعة",
      });
  ...
  async function onConfirmReverse() {
    ...
    if (error) {
      return toast.error("فشل عكس المصروف", { description: error.message });
    }
  ...
  return (
    <div>
      <PageHeader
        title="المصروفات"
        description="تسجيل المصروفات مع تخصيص مصادر التمويل تلقائياً"
        actions={canCreate && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent dir="rtl" className="max-w-2xl">
              ...
              <form onSubmit={onSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="space-y-2 rounded-md border p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          variant="outline"
                          size="sm"
                          onClick={() => setAllocations([...allocations, { funding_check_id: "", amount: "" }])}
                        >
                      ...
                      {allocations.map((a, i) => {
                        ...
                        return (
                          <div key={i} className="grid grid-cols-[1fr_140px_auto] gap-2 items-end">
                            <div>
                              <Select
                                value={a.funding_check_id}
                                onValueChange={(v) => {
                                  const next = [...allocations];
                                  next[i] = { ...next[i], funding_check_id: v };
                                  setAllocations(next);
                                }}
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="اختر صك تمويل" />
                                </SelectTrigger>
                                <SelectContent>
                                  {(checks ?? []).map((x: any) => {
                                    ...
                                    return (
                                      <SelectItem key={x.id} value={x.id}>
                                        صك {x.check_number} — {x.funders?.name} — {x.cash_accounts?.name} — متبقي {formatCurrency(r)}
                                      </SelectItem>
                                    );
                                  })}
                                </SelectContent>
                              ...
                              {c && (
                                <div className="text-[11px] text-muted-foreground mt-1">
                                  المتبقي: <span className="tabular-nums">{formatCurrency(rem)}</span>
                                </div>
                              )}
                            </div>
                      ...
                      <div className={`flex justify-between text-sm pt-2 border-t ${allocMismatch ? "text-destructive" : "text-muted-foreground"}`}>
                        ...
                        <span className="tabular-nums font-medium">
                          {formatCurrency(allocTotal)} / {formatCurrency(amountNum)}
                        </span>
                      </div>
                    </div>
                ...
                <DialogFooter>
                  <Button type="submit" disabled={busy || allocMismatch}>
                    {busy ? "جاري الحفظ..." : "حفظ المصروف"}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
                  ...
                  <TableBody>
                    {filtered.map((e: any) => (
                      <TableRow key={e.id}>
                        ...
                        <TableCell>
                          {e.expense_scope === "project" && (
                            <>
                              <div className="font-medium">
                                {e.projects?.name ?? (
                                  <Badge variant="destructive">مشروع مفقود</Badge>
                                )}
                              </div>
                          ...
                          {e.expense_scope === "asset" && (
                            <>
                              <div className="font-medium">
                                {e.assets?.asset_name ?? (
                                  <Badge variant="destructive">أصل مفقود</Badge>
                                )}
                              </div>
                        ...
                        <TableCell className="text-xs">
                          {e.payment_status === "payable" ? (
                            ...
                            Array.from(
                              new Set(
                                (e.expense_funding_allocations ?? [])
                                  .map((a: any) => a.funding_checks?.cash_accounts?.name)
                                  .filter(Boolean)
                              )
                            ).join("، ") || "—"
                          )}
                        ...
                        <TableCell className="tabular-nums text-xs whitespace-nowrap" dir="ltr">
                          {(e.expense_funding_allocations ?? [])
                            .map((a: any) => a.funding_checks?.check_number)
                            .filter(Boolean)
                            .join("، ") || "—"}
                        </TableCell>
                        ...
                        <TableCell className="text-left font-medium tabular-nums whitespace-nowrap">
                          ...
                          <div className="flex gap-1">
                            {e.attachment_url && (
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => downloadAttachment(e.attachment_url)}
                                title="مرفق"
                              >
                            ...
                            {e.excel_attachment_url && (
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => downloadAttachment(e.excel_attachment_url)}
                                title="ملف Excel"
                              >
      ...
      <Dialog open={!!editingExp} onOpenChange={(o) => !o && setEditingExp(null)}>
        <DialogContent dir="rtl" className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={onSaveEditExp} className="space-y-4">
            ...
            <div className="space-y-2 rounded-md border p-3">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => setEditAllocations([...editAllocations, { funding_check_id: "", amount: "" }])}
                >
                  <Plus className="size-3.5" />
                  صك آخر
                </Button>
              ...
              {editAllocations.map((a, i) => (
                <div key={i} className="grid grid-cols-[1fr_140px_auto] gap-2 items-end">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="اختر صك" />
                    </SelectTrigger>
                    <SelectContent>
                      {(checks ?? []).map((x: any) => (
                        <SelectItem key={x.id} value={x.id}>
                          صك {x.check_number} — {x.funders?.name} — {x.cash_accounts?.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <div className={`flex justify-between text-sm pt-2 border-t ${editMismatch ? "text-destructive" : "text-muted-foreground"}`}>
                <span>مجموع التخصيصات</span>
                <span className="tabular-nums font-medium">
                  {formatCurrency(editAllocTotal)} / {formatCurrency(editAmountNum)}
                </span>
              </div>
            </div>
            ...
            <DialogFooter>
              <Button type="submit" disabled={editBusy || editMismatch}>
                {editBusy ? "جاري الحفظ..." : "حفظ التعديل"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
